...
梯度下降相关介绍:
    ...
    自适应矩估计 Adam：即优化学习率，又优化梯度
        公式：
            一阶矩：算均值
                Mt = β1 * Mt-1 + (1 - β1) * Gt        梯度优化
                St = β2 * St-1 + (1 - β2) * Gt * Gt   学习率优化
            二阶矩：梯度方差
                Mt^ = Mt / (1 - β1 ^ t)
                St^ = St / (1 - β2 ^ t)
            Adam权重更新公式：
...
def dm01():
    ...
    # AdaGrad :
    optimizer = optim.adagrad(param = [w], lr=0.01)

    # rmsprop : alpha调和权重系数
    optimizer = optim.rmsprop(param = [w], lr=0.01, alpha=0.9)

    # adam : betas (梯度， 学习率)
    optimizer = optim.adam(param = [w], lr=0.01, betas=(0.9, 0.999),)
    # todo: 3-计算梯度值
